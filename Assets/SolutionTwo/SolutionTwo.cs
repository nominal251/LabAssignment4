...
public class SolutionTwo : MonoBehaviour
{
    ...
    void Start()
    {
        GameObject holder = Instantiate(character, transform.position, Quaternion.identity);
        instance = holder.GetComponent<Template>();

        EstablishDictionaries();

        dice = diceValues[instance.characterClass];

        if (instance.averaged)
        {
            AveragedCalc();
    ...
    void AveragedCalc()
    {
        ...
        for (int i = 0; i < instance.level; i++)
        {
            hp += expectedValue;
    ...
    void CheckStats()
    {
        if (instance.characterRace == "dwarf")
        {
            hp += 2f;
        }
        else if (instance.characterRace == "orc" || instance.characterRace == "goliath")
        {
            hp += 1f;
        }
        ...
        hp += conValues[instance.conScore];
    }
